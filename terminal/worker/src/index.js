...
function renderMedia(){
  ...
  uploadedMedia.forEach((item,index)=>{
    ...
    actions.append(hero,left,right,remove);meta.append(name,actions);card.append(thumb,badge,meta);
  ...
  try{
    for(const original of queue){
      ...
      try{
        ...
        uploadedMedia.push({url:d.url,name:original.name,preview,hero:uploadedMedia.length===0});
        renderMedia();

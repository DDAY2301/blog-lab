...
QUEUE = BASE / "data/deferred-articles.json"
...
def read_payload(command_file: str) -> dict:
    ...
    if not isinstance(data, dict):
        raise SystemExit("Deferred article payload must be a JSON object.")
    return data


def compact_reason(value: str, limit: int = 1600) -> str:
    text = " ".join(str(value or "").split())
    return text[-limit:]


def is_writer_capacity_log(value: str) -> bool:
    low = str(value or "").lower()
    return (
        "article_ai_unavailable" in low
        or "ai pisec" in low
        or "workers ai writer" in low
        or "writer ni uspel" in low
        or "bad gateway" in low
        or "http error 502" in low
...
def append_deferred(command_file: str, request_id: str, reason: str) -> int:
    ...
    if not command:
        raise SystemExit("Cannot defer an empty article command.")
    ...
    print(f"DEFERRED_ARTICLE_QUEUED id={request_id} pending={pending}")
    return 0


def command_file_for(entry: dict, workdir: Path) -> Path:
    workdir.mkdir(parents=True, exist_ok=True)
    path = workdir / f"deferred-article-{entry.get('id', 'manual')}.json"
    payload = {
        "request_id": entry.get("id"),
        "command": entry.get("command"),
        "mode": "article",
        "category": entry.get("category") or "aktualno",
        "actor": entry.get("actor") or "deferred-articles",
        "created_at": entry.get("created_at") or utc_now(),
    }
...
def replay(max_items: int) -> int:
    ...
    if not pending:
        print("DEFERRED_ARTICLE_NONE")
        return 0

    workdir = Path(os.environ.get("RUNNER_TEMP") or (BASE / ".tmp")) / "deferred-articles"
    processed = 0
    resolved = 0
    still_pending = 0
    ...
    for entry in pending[: max(1, max_items)]:
        ...
        if proc.returncode == 0:
            ...
            resolved += 1
            print(f"DEFERRED_ARTICLE_RESOLVED id={entry.get('id')} attempts={entry.get('attempts')}")
        elif proc.returncode == 64 and is_writer_capacity_log(log):
            entry["status"] = "pending"
            entry["reason"] = "AI writer capacity/provider unavailable; will retry later."
            still_pending += 1
            print(f"DEFERRED_ARTICLE_STILL_PENDING id={entry.get('id')} attempts={entry.get('attempts')}")
        else:
            entry["status"] = "failed"
            entry["failed_at"] = utc_now()
            entry["reason"] = log or f"command.py exited with {proc.returncode}"
            print(f"DEFERRED_ARTICLE_FAILED id={entry.get('id')} rc={proc.returncode}")

    save_queue(items)
    print(f"DEFERRED_ARTICLE_SUMMARY processed={processed} resolved={resolved} still_pending={still_pending}")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(description="Queue and replay Blog Lab manual articles deferred by AI writer capacity limits.")
